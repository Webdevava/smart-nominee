...
import AddTravelInsuranceDialog from "@/components/dialogs/add-travel-insurance"
...
const travelInsurances = [
  {
    ...
    owner: "Father",
    details: [
      { label: "Travel Type", value: "Domestic Travel" },
      { label: "Policy Number", value: "123456" },
      { label: "Insurance Provider", value: "LIC" },
      ...
      { label: "Coverage Details", value: "Medical Emergencies, Lost Baggage, Flight Delays / Cancellations" },
    ],
    nominees: [
...
const TravelInsuranceCard = ({ insurance }) => (
  <Card className="p-0">
    <CardHeader>
...
const TravelInsuranceTab = () => {
  const [dialogOpen, setDialogOpen] = useState(false)

  return (
    <div className="p-2 sm:p-6 mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-6">
        <h1 className="text-lg sm:text-xl font-semibold">Total Travel Insurance ({travelInsurances.length})</h1>
        <Button className="w-full sm:w-auto" onClick={() => setDialogOpen(true)}>
          <PlusCircle className="mr-2 h-4 w-4" /> Add Insurance
        </Button>
      </div>
      <AddTravelInsuranceDialog open={dialogOpen} onOpenChange={setDialogOpen} />
      <div className="space-y-6">
        {travelInsurances.map((insurance, index) => (
          <TravelInsuranceCard key={index} insurance={insurance} />
        ))}
      </div>
    </div>
  )
...
export default TravelInsuranceTab
