...
import AddVehicleInsuranceDialog from "@/components/dialogs/add-vehicle-insurance"
...
const vehicleInsurances = [
  {
    ...
    details: [
      ...
      { label: "Vehicle Registration Number", value: "1234567890" },
...
const VehicleInsuranceCard = ({ insurance }) => (
  <Card className="p-0">
    <CardHeader>
...
const VehicleInsuranceTab = () => {
  const [dialogOpen, setDialogOpen] = useState(false)

  return (
    <div className="p-2 sm:p-6 mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-6">
        <h1 className="text-lg sm:text-xl font-semibold">Total Vehicle Insurance ({vehicleInsurances.length})</h1>
        <Button className="w-full sm:w-auto" onClick={() => setDialogOpen(true)}>
          <PlusCircle className="mr-2 h-4 w-4" /> Add Insurance
        </Button>
      </div>
      <AddVehicleInsuranceDialog open={dialogOpen} onOpenChange={setDialogOpen} />
      <div className="space-y-6">
        {vehicleInsurances.map((insurance, index) => (
          <VehicleInsuranceCard key={index} insurance={insurance} />
        ))}
      </div>
    </div>
  )
