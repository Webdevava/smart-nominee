...
import AddHealthInsuranceDialog from "@/components/dialogs/add-health-insurance"
...
const healthInsurances = [
  {
    ...
    owner: "Father",
    details: [
      { label: "Policy Type", value: "Family" },
...
const HealthInsuranceCard = ({ insurance }) => (
  <Card className="p-0">
    <CardHeader>
  ...
  return (
    <div className="p-2 sm:p-6 mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-6">
        <h1 className="text-lg sm:text-xl font-semibold">Total Health Insurance ({healthInsurances.length})</h1>
        <Button className="w-full sm:w-auto" onClick={() => setDialogOpen(true)}>
          <PlusCircle className="mr-2 h-4 w-4" /> Add Insurance
        </Button>
      </div>
      <AddHealthInsuranceDialog open={dialogOpen} onOpenChange={setDialogOpen} />
      <div className="space-y-6">
        {healthInsurances.map((insurance, index) => (
          <HealthInsuranceCard key={index} insurance={insurance} />
        ))}
      </div>
    </div>
  )
