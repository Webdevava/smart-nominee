...
import AddPropertyInsuranceDialog from "@/components/dialogs/add-property-insurance"
...
const propertyInsurances = [
  {
    ...
    owner: "Father",
    details: [
      { label: "Policy Type", value: "Home" },
      { label: "Policy Number", value: "123456" },
      { label: "Insurance Provider", value: "Bajaj Allianz" },
      ...
      { label: "Linked Phone Number", value: "1234567890" },
      {
        ...
        value: "Room No. 12, Shanti Niwas PG, Opposite Oberoi Mall, Goregaon East, Mumbai, Maharashtra, 400063",
      },
      { label: "Coverage Details", value: "Both Structure & Contents" },
    ],
    nominees: [
...
const PropertyInsuranceCard = ({ insurance }) => (
  <Card className="p-0">
    <CardHeader>
...
const PropertyInsuranceTab = () => {
  const [dialogOpen, setDialogOpen] = useState(false)

  return (
    <div className="p-2 sm:p-6 mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-6">
        <h1 className="text-lg sm:text-xl font-semibold">Total Property Insurance ({propertyInsurances.length})</h1>
        <Button className="w-full sm:w-auto" onClick={() => setDialogOpen(true)}>
          <PlusCircle className="mr-2 h-4 w-4" /> Add Insurance
        </Button>
      </div>
      <AddPropertyInsuranceDialog open={dialogOpen} onOpenChange={setDialogOpen} />
      <div className="space-y-6">
        {propertyInsurances.map((insurance, index) => (
          <PropertyInsuranceCard key={index} insurance={insurance} />
        ))}
      </div>
    </div>
  )
